...
const blockuser = async (req,res)=>{
    try {
        ...
        const userblock = await User.findOneAndUpdate({_id:id,isActive:1},{$set:{isActive:0}})
...
const unblockuser = async (req,res)=>{
    try {
        const id = req.query.id
        const userunblock = await User.findOneAndUpdate({_id:id,isActive:0},{$set:{isActive:1}})
