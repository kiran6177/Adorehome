...
const loaduserview = async (req, res) => {
  try {
    ...
    const udata = await User.find({ type: "user" }).skip((page - 1)* 8).limit(8);
    const userCount = await User.aggregate([{$match:{type:'user'}},{$count:'usercount'}])
    console.log(userCount)

    res.render("admin/userview", { udata: udata ,totalusercount : userCount.length > 0 ? userCount[0].usercount : 0});
...
const userSearch = async (req,res)=>{
  try {
    ...
    const userCount = await User.aggregate([{$match:{$and:[{type:"user"},{$or:[{firstname:{$regex: new RegExp(key,'i')}},{lastname:{$regex:new RegExp(key,'i')}}]}]}},{$count:'usercount'}])
    if(userdata.length > 0)
    {
      console.log(userCount)
      res.render('admin/userview',{udata:userdata,totalusercount : userCount.length > 0 ? userCount[0].usercount : 0})
    }
    else{
...
const pageSearch = async (req,res)=>{
  try {
    ...
    if(search!=null)
    {
      udata = await User.find({$and:[{type:"user"},{$or:[{firstname:{$regex: new RegExp(search,'i')}},{lastname:{$regex:new RegExp(search,'i')}}]}]}).skip((page - 1)* 8).limit(8);

    }else{
       udata = await User.find({ type: "user" }).skip((page - 1)* 8).limit(8);
    }
    console.log(udata)
    if(udata.length > 0)
    {
