...
const loadhome = async (req,res)=>{
    ...
    const prodet = await Product.aggregate([{$match:{isBlocked:0}},{$sort:{date:-1}},{$limit:8}])
    const prodetCount = await Product.aggregate([{$match:{isBlocked:0}},{$sort:{date:-1}},{$count:'stock'}])
    ...
    const totalProCount = await Order.aggregate([{$match:{payment_status:"Paid"}},{$unwind:'$products'},{$lookup:{from:'users',localField:'user_id',foreignField:'_id',as:'userdetails'}},{$lookup:{from:'products',localField:'products.product_id',foreignField:'_id',as:'prodetails'}},{$count:'proCount'}])
    let currentPage = propage
    //  console.log(totalProCount[0].proCount)
    res.render("admin/dashboard",{userCount:ucount,orderCount:ocount,productCount:pcount,products:prodet,prosold:proSold,totalProducts:totalProCount[0].proCount,currentPage:currentPage,totalStock:prodetCount[0].stock})
    }
    catch(err)
    {
