...
const loadProductDetail = async (req, res) => {
  try {
    ...
    const productdetail = await Product.findById({ _id: proid }).populate(
      "offer_id"
    );
    const udata = await User.findById({ _id: uid }).populate("cart.product_id");
...
const proSearch = async (req, res) => {
  try {
    ...
    if (catid) {
      console.log("cat");
      pdata = await Product.aggregate([
        {
          $match: {
            productname: { $regex: new RegExp(search, "i") },
            category_id: new ObjectId(catid),
          },
        },
        {
          $lookup: {
    ...
    const pdatacount = await Product.aggregate([
      { $match: { productname: { $regex: new RegExp(search, "i") } } },
      { $count: "pcount" },
    ]);
  ...
  try {
    ...
    if (
      ...
      pdata = await Product.aggregate([
        ...
        { $sort: { date: filterbydate } },
        { $skip: (page - 1) * 8 },
      ]);
      pdatacount = await Product.aggregate([
        {
          $match: {
            productname: { $regex: new RegExp(search, "i") },
            brand_id: new ObjectId(brandid),
