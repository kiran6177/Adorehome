...
const loadPayment = async (req, res) => {
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      }).populate({ path: "category_id", match: { status: "1" } });
...
const paymentConfirm = async (req, res) => {
  try {
    ...
    const orderdata = {
      ...
      products: products,
      coupon_id:
        couponid != null ? (couponid != "null" ? couponid : "Nil") : "Nil",
    ...
    if (couponid != null && couponid != "null") {
      const addcoupontoUser = await User.findByIdAndUpdate(
        { _id: uid },
        { $push: { coupon_id: couponid } }
      );
      if (addcoupontoUser) {
