...
const addBanner = async (req,res)=>{
    try {
        ...
        console.log(bannertitle,description,status,req.file.filename,croppeddata)
        ...
        if(croppedbanner!=null){
            ...
            cropAndSave(inputImagePath, outputImagePath, croppedbanner.x, croppedbanner.y, croppedbanner.width, croppedbanner.height);
          }
...
const loadEditBanner = async (req,res)=>{
    try {
        const {id} = req.query
        console.log(id)
        const bannerdata = await Banner.aggregate([{$match:{_id:new ObjectId(id)}}])
...
const editBanner = async (req,res)=>{
    try {
        ...
        const bannerEdit = await Banner.findByIdAndUpdate({_id:bannerid},{$set:bannerToedit},{new:true})
