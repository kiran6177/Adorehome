...
usearchbtn.addEventListener('click',()=>{
    ...
    proSearch(searchvalue)
})

profilter.addEventListener('change',()=>{
    let filtervalue = profilter.value
    let searchvalue = usearch.value
    console.log("Se in F"+searchvalue)
    console.log(filtervalue)
    fetchFiltered(searchvalue,filtervalue)
})

async function fetchFiltered(search,filter){
    try {
        const res = await fetch(`/products/filterpro?search=${search}&filter=${filter}`)
...
async function proSearch(usearch)
{
    try{
    const res = await fetch(`/products/searchpro?search=${usearch}`)
    const data = await res.json()
    if(data.products)
    {
        console.log(data)
