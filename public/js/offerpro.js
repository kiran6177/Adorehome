...
profilter.addEventListener('change',()=>{
    ...
    let searchvalue = usearch.value
    let offerid = document.getElementById('offerid').value
    console.log("Se in F"+offerid)
    console.log(filtervalue)
    fetchFiltered(searchvalue,filtervalue,offerid)
})

async function fetchFiltered(search,filter,offerid){
    try {
        const res = await fetch(`/products/filterpro?search=${search}&filter=${filter}&offerid=${offerid}`)
...
async function proSearch(usearch,offerid)
{
    try{
    const res = await fetch(`/products/searchpro?search=${usearch}&offerid=${offerid}`)
    const data = await res.json()
    if(data.products)
    {
        injection(data)
