...
profilter.addEventListener('change',()=>{
    ...
    let searchvalue = usearch.value
    let catid = document.getElementById('catid').value
    console.log("Se in F"+catid)
    console.log(filtervalue)
    fetchFiltered(searchvalue,filtervalue,catid)
})

async function fetchFiltered(search,filter,catid){
    try {
        const res = await fetch(`/products/filterpro?search=${search}&filter=${filter}&catid=${catid}`)
...
async function proSearch(usearch,catid)
{
    try{
    const res = await fetch(`/products/searchpro?search=${usearch}&catid=${catid}`)
    const data = await res.json()
    if(data.products)
    {
        injection(data)
