...
profilter.addEventListener('change',()=>{
    ...
    let searchvalue = usearch.value
    let brandid = document.getElementById('brandid').value
    console.log("Se in F"+brandid)
    console.log(filtervalue)
    fetchFiltered(searchvalue,filtervalue,brandid)
})

async function fetchFiltered(search,filter,brandid){
    try {
        const res = await fetch(`/products/filterpro?search=${search}&filter=${filter}&brandid=${brandid}`)
...
async function proSearch(usearch,brandid)
{
    try{
    const res = await fetch(`/products/searchpro?search=${usearch}&brandid=${brandid}`)
    const data = await res.json()
    if(data.products)
    {
        injection(data)
